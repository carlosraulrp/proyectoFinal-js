...
function miProyecto(productos) {
  ...
  function renderizarProducto(arrayProductos) {
    ...
    arrayProductos.forEach((element) => {
      //recorro el array que cree como parametro en la funcion
      ...
      cardProducto.innerHTML += `                              
        ...
        <button id=${
          element.id
        } type="button" class="btn btn-primary">AGREGAR AL CARRITO</button>
  ...
  function filtrar() {
    let arrayFiltradoBuscador = productos.filter(
      (element) =>
        element.nombre.includes(buscador.value.toLowerCase()) ||
        element.deporte.includes(buscador.value.toLowerCase())
    );
  ...
  function filtrarCategoria(e) {
    let arrayCategorias = productos.filter(
      (element) => element.deporte == e.target.id
    );
  ...
  botonIngresar.addEventListener("click", () => {
    ...
    if (
      infoUsuario.usuario == usuarioLg.value &&
      infoUsuario.clave == claveLg.value
    ) {
      Swal.fire({
        position: "top-end",
        icon: "success",
